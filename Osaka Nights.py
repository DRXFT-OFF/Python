...
def game():
    ...
    stats['Траты'] = 10000    # выставляем как оно должно было быть в обучении
    sleep(2)
    print('-' * 20)
...
def shop():         # функция магазина
    ...
    if close == 'Закрыть':                    # если ввод правильный, то двигаемся к магазину
        ...
        sleep(1 if hard == 1 else 60)
        print('Вы пришли на место!')
    bill = 0  # переменная для подсчёта суммы покупок
    while True:  # начинаем цикл покупок
        print('-' * 30)
        print(f'Продукты: \n{products}')    # пишем продукты, сумму на руках, цены и чек
        print('-' * 15)
        print(f'У вас: \n{stats["¥"]}¥')
        print('-' * 15)
        print(f'Чек: {bill}¥')
        ...
        if ans == 'сосиски':  # добавление продуктов в корзину и добавление цены
            try:    # пытаемся ввести кол-во сколько купить продукта
            ...
            except ValueError:      # если вводят букву:
                bill = shopping(buy_ex(), bill, ans)  # вызываем функцию покупки со встроенной отладкой
        elif ans == 'чипсы':
            try:    # пытаемся ввести кол-во сколько купить продукта
            ...
            try:    # пытаемся ввести кол-во сколько купить продукта
                numb = int(input(f'Сколько кг купить? 0 - отмена \n'))
                if numb == 0:  # функция отмены покупки продукта при введённом 0
                    continue
                bill = shopping(numb, bill, ans)  # вызываем функцию хитрожоп-отладки и покупки
            except ValueError:      # если вводят букву:
...
def shopping(numb_ex, all_bill, ans_1):       # функция хитрожоп-отладки и покупки продуктов
    while numb_ex < 0:      # пока число сколько хотим купить продуктов < 0:
        try:        # пытаемся ввести верное число
            numb_ex = int(input('Нельзя купить отрицательное кол-во. 0 - отмена '))
            if numb_ex == 0:    # если ввели 0, то возвращаемся к покупкам
                break
        except ValueError:      # если вводят букву:
            numb_ex_fx = buy_ex()      # присваиваем значение с функции как кол-во продукта чтобы не вызывать её 2 раза
            all_bill += numb_ex_fx * prices[ans_1]     # к сумме прибавляем кол-во продукта * цену продукта
            products[ans_1] += numb_ex_fx      # кладём в корзину нужное кол-во продукта
            return all_bill         # возвращаем весь счёт для корректного суммирования
    all_bill += numb_ex * prices[ans_1]      # к сумме прибавляем кол-во продукта * цену продукта
    products[ans_1] += numb_ex      # кладём в корзину нужное кол-во продукта
    return all_bill     # возвращаем весь счёт для корректного суммирования


def buy_ex():         # отладка ввода букв
    ex_num = None
    while type(ex_num) != int:  # пока не введут число:
        try:  # пытаемся ввести число
            ex_num = int(input('Введено неверное число. Сколько хотите купить? 0 - отмена '))
            if ex_num == 0:  # если ввели 0, то возвращаемся к выбору продукта
                break
            while ex_num < 0:   # пока число < 0:
                try:    # пытаемся ввести число > 0
                    ex_num = int(input('Нельзя купить отрицательное кол-во. 0 - назад '))
                    if ex_num == 0:     # если введено 0, то возвращаемся к покупкам
                        break
                except ValueError:  # если введена буква:
                    ex_num = None   # присваиваем ничего
                    while type(ex_num) != int:      # пока не будет введено число
                        try:    # пытаемся ввести число
                            ex_num = int(input('Введено неверное число. Сколько хотите купить? 0 - отмена '))
                            if ex_num == 0:  # если ввели 0, то возвращаемся к выбору продукта
                                break
                        except ValueError:  # если ввели букву, то продолжаем запрашивать число
                            continue
        except ValueError:  # если вводят букву, то просим ввести число ещё раз
            continue
    return ex_num        # возвращаем число для последующей покупки
...
print('-' * 10)
sleep(2)
print('Список изменений: \n-Игра была немного оптимизирована \n-Добавлена локация дома, '
      'возможность удовлетворять  потребности и ходить на работу'
      '\n-Цены на продукты изменены и молоко было заменено на удон \n-Были исправлены некоторые баги в магазине')
print('-' * 20)
sleep(5)
...
while tutorial != 'Д' and tutorial != 'Н':      # пока не будет равняться Д или нет:
    tutorial = input('Неверный ввод. Пропустить обучение? Д/Н ').capitalize()   # будем запрашивать
if tutorial == 'Н':
    print('--------------- \nОБУЧЕНИЕ \n---------------')
...
if tutorial == 'Д':
    game()
print('Благодарим за то, что опробовали Osaka Nights Alpha 0.3.1')
sleep(1)
